...
import { pool } from "../db"
import { sendMail } from "../EmailService/Nodemailer";
...
const getProductsAccordingTags = async (req:Request, res: Response) => {
  try{ 
...
const verifyOtp = async(req:Request, res:Response) => {
  ...
  try {
    ...
    const result = await pool.query("SELECT otp FROM email_verification WHERE email = $1", [email]);
    ...
    await pool.query(
      "UPDATE email_verification SET is_verified = true, verified_at = NOW() WHERE email = $1",[email]
    )
...
const placeOrder = async (req:Request, res:Response) => {
  ...
  const summary = items.map((item:any) => `• ${item.name} - $${item.price}`)
...
export { AddProducts, getAllProducts, getProductsAccordingTags, sendOtp, verifyOtp , placeOrder , getProductById }